...
Now, we have `r nrow(scenarios)` scenarios and I don't want to run a for loop, I want to parallelize them ([embarrasingly](http://en.wikipedia.org/wiki/Embarrassingly_parallel)).  I now want to create what's called an <strong>array job</strong>.  Now our Rscript.sh would be:
```
#!/bin/bash 
#$ -t 1-`r nrow(scenarios)`
R CMD BATCH Rscript.R
```
So that `r nrow(scenarios)` jobs are run when 
```
qsub /path/to/script/Rscript.sh
```
